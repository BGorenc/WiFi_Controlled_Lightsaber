...
- 1x [Power Supply 5v 30 Amp](https://www.amazon.com/dp/B0BNQ6Y88J?ref=ppx_yo2ov_dt_b_product_details&th=1)
    - This transformer will be used to supply the power and amps needed for both LED strips. This power supply will provide a large buffer on your power demands as you shouldn't be using more than 20 Amps. Conservatively, 60 mA per LED and max of 200 mA for the Arduino Nano 33 IoT when using Wi-Fi <= 18 Amps.
...
- 1x [>=100 uf 25v Capacitor](https://www.arduino.cc/documents/datasheets/cap100uF.pdf)
    - This serves as a way to maintain a constant power supply. This shouldn't be needed for this power supply but easy enough to add. Many options available for purchase.
...
- 1x [Foam tube for 1" thin tubes V2](https://www.thecustomsabershop.com/Foam-tube-for-1-thin-tubes-V2-P1336.aspx)
    - This method will help diffuse the light emitted by the interconnected LED strips, which are inserted into this foam tubing.
...
- 1x [20 awg Stranded Silicone Wire 23 ft Black and Red](https://www.amazon.com/dp/B08F79YG8Q?psc=1&ref=ppx_yo2ov_dt_b_product_details)
    - I used this to run the 5V power lines on the Arduino board. This size is probably not necessary but I like to keep a factor of safety when running power from a large transformer.
...
- 1x [PCB Board](https://www.amazon.com/gp/product/B07ZYNWJ1S/ref=ppx_yo_dt_b_search_asin_title?ie=UTF8&th=1)
    - You only need one for this project and the quality of this PCB is perfect for what you will need since it mirrors a breadboard layout. Feel free to find another variety.
...
- 1x [Dremel Tool Kit](https://www.homedepot.com/p/Dremel-3000-Series-1-2-Amp-Variable-Speed-Corded-Rotary-Tool-Kit-with-25-Accessories-and-Carrying-Case-3000-1-25H/203040434)
    - This will make your sanding much easier and give you the ability to easily cut a wedge out of your spacer for wire strain relief.
- 1x [Glue Gun](https://www.amazon.com/Mr-Stick-Crafts-Craft-Sticks/dp/B08BCCS1SN/ref=sr_1_10?c=ts&keywords=Craft+Glue+Guns&qid=1684882690&s=arts-crafts&sr=1-10&ts_id=12898851)
    - Can be used to help hold the two LED strips together before being inserted into the foam tubing. Though the strips do ship with an adhesive tape on them, I prefer to remove that.
...
- 1x [Cord Covers](https://www.amazon.com/dp/B08MT3ZXSD?psc=1&ref=ppx_yo2ov_dt_b_product_details)
    - Hide your loose wires so the final product looks more appealing when mounted. To fully hide the wires I replaced all of the WAGO connectors at the base of the blade with a solder connection and shrink tubing. I soldered connections to the JST connector that mates with the JST connector that is attached to the LED strips.
...
<ins>**18 Awg:**</ins>
    - Used to run power directly from the power supply to the LED strips and the Arduino
...
When powered on in the setup() section the program initializes the LED strips, sets the brightness and power limits, and starts the serial communication. However, the most critical component is setting the IP address of the Arduino. This makes sure you always have a fixed IP address for your Arduino, rather than allowing it to be assigned a random IP address. Even if you went through the trouble of figuring it out there is no guarantee your router will always assign the same IP. Once these steps are done, then the arduino attempts to connect to the configured WiFi network using the WiFiNINA module. Once connected, it starts a server on port 80 to host a small simple web interface accessible by placing the IP address you set into the URL of any browser on the same network. If connection is lost the code will attempt to reconnect to the WiFi. 
